...
def populate_train_list(orig_images_path, hazy_images_path):
    ...
    for extension in IMG_EXTENSIONS:
        image_list_haze.extend(glob.glob(hazy_images_path + '*' + extension))
    tmp_dict = {}

    for image in image_list_haze:
        image = image.split("/")[-1]
        key = image.split("_")[0] + os.path.splitext(image)[-1]  # ".png"
...
class dehazing_loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # resize to 512x512 here, so there is no need to judge whether H and W are multiples of 4
        if config.height % 4 != 0 or config.width % 4 != 0:
            H, W = data_orig.shape
            data_orig = cv2.resize(data_orig, (int(W // 4 * 4), int(H // 4 * 4)))
            data_hazy = cv2.resize(data_hazy, (int(W // 4 * 4), int(H // 4 * 4)))
