...
# from ..utils import conv1x1, conv3x3, CRPBlock
import os, sys
...
def new_rf_lw50(num_classes, pretrain=True, ckpt_path=None, **kwargs):
    model = ResNetLW(Bottleneck, [3, 4, 6, 3], num_classes=num_classes, **kwargs)
    if pretrain:
...
def new_rf_lw101(num_classes, pretrain=True, ckpt_path=None, **kwargs):
    model = ResNetLW(Bottleneck, [3, 4, 23, 3], num_classes=num_classes, **kwargs)
    if pretrain:
        ckpt = torch.load(ckpt_path)
        model.load_state_dict(ckpt)
...
def new_rf_lw152(num_classes, pretrain=True, ckpt_path=None, **kwargs):
    model = ResNetLW(Bottleneck, [3, 8, 36, 3], num_classes=num_classes, **kwargs)
    if pretrain:
        ckpt = torch.load(ckpt_path)
        model.load_state_dict(ckpt)
